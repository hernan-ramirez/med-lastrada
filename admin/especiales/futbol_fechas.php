...
if(isset($agregar_actualizar)){
	for($i=0; $i < count($fechas); $i++){
		...
		$result_exist = mysql_query("SELECT id FROM futbol_partidos WHERE fecha_partido = '$partes[0]' && id_primer_club = $partes[1] && id_segundo_club = $partes[2] && id_torneo = $id_torneo && numero_fecha = $fecha");
		if(mysql_num_rows($result_exist)==0){
			mysql_query("INSERT INTO futbol_partidos (fecha_partido,id_primer_club,id_segundo_club,id_torneo,numero_fecha)VALUES('$partes[0]',$partes[1],$partes[2],$id_torneo,$fecha)");
		}
	}
...
function pasar(){
	...
	nuevo.text = fecha_format + ") " + it + " - " + et ;
	nuevo.value = fecha + "|" + iv + "|" + ev;
...
function agregarVinculado(tabla){
	archivo = "<? if(isset($path_de_relacion)){ echo $path_de_relacion;} ?>ow.php?accion=agregar&tabla=futbol_clubes&base=<? echo $base ?>";
	nombre_ventana = "agregar_club";
...
if($id_torneo!="" && $fecha!=""){ 
	...
	if(mysql_num_rows($result)!=0){
		...
		while($row=mysql_fetch_array($result)){
			?>
                <OPTION value="<? echo $row[3] . "|" . $row[4] . "|" . $row[5] ?>" style="background-color:#<? echo $color ?>"><? echo $row[0] . ") " . $row[1]. " - " . $row[2] ?></OPTION>
                <?
