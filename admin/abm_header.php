...
<TR> 
	<TD width="33%"><FONT color="#FFFFFF"><B>&nbsp;<? echo strtoupper($tabla) ?></B><? if(isset($id)){ 
    ...
    <TD nowrap class="LiteViolet">
	   ...
	   <TR> 
		...
		<INPUT type="hidden" name="tiene_solapas" value="si">
			 ...
			 <TD class="BackSolapa<? echo $img_estado ?>"><A href="?tabla=<?  echo $row_rel["tabla"] ?>&include=abm&id=<? echo $id ?>&accion=modificar"><? echo $row_rel["nombre"] ?></A></TD>
			...
			?>
			 ...
			 <TD><IMG src="image/solapa_1_<? echo $img_estado ?>.gif" width="2" height="25"></TD>
			 <TD class="BackSolapa<? echo $img_estado ?>"><A href="?tabla=<?  echo $tabla ?>&include=<? echo $row_solapas["include"] ?>&id=<? echo $id ?>"><? echo $row_solapas["nombre"] ?></A></TD>
			 <TD><IMG src="image/solapa_2_<? echo $img_estado ?>.gif" width="1" height="25"></TD>
			 <?
			} ## fin result_solapas
